<div
    class="grid gap-1 justify-start [grid-template-columns:repeat(2,42px)]">
    @for ($bagSlotId = 2500; $bagSlotId <= 2501; $bagSlotId++)
        @php
            $inv = $items['shared_bank']->firstWhere('slot_id', $bagSlotId);
            $bagSize = $inv->item->bagslots ?? 0;
            $startSlotId = 11010 + ($bagSlotId - 2500) * 200;

            $itemsBySlotId = $items['shared_bank_items']
