@if ($character->anon >= 1)
    <div class="relative">
        <div class="justify-start">
            <x-ui.alert-info>
                This character is set to Anonymous or Roleplay, so bank inventory is hidden.
            </x-ui.alert-info>
        </div>
    </div>
@else
    <div class="relative">
        ...
        @for ($bagSlotId = 2000; $bagSlotId <= 2023; $bagSlotId++)
            @php
                $inv = $items['bank']->firstWhere('slot_id', $bagSlotId);
                //$bagSlotId = $inv->slot_id;
                $bagSize = $inv->item->bagslots ?? 0;
                $startSlotId = 6210 + ($bagSlotId - 2000) * 200;

                $itemsBySlotId = $items['bank_items']
